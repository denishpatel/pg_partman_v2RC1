-- IMPORTANT NOTE: This version, and all future versions of pg_partman >= 2.0.0, are only compatible with PostgreSQL 9.4 and greater.
    -- I do not have any current plans for development on the v1.x.x branch anymore. Reported bugs will be fixed, but all development will only be on the 2.x.x from here on out. Bug fixes will also only be done while versions of PostgreSQL older than 9.4 are themselves officially supported. Once PostgreSQL 9.3 goes EOL, the 1.8 series will be deprecated entirely.
    -- No new features, API changes, configuration options or major internal structure changes will be accepted for the 1.x.x series. These will be locked to the 1.8.x series. No future 1.x.x versions after 1.8.4 will be able to be installed directly. You will have to install 1.8.4 (tagged version releases are available on github) and then install the updates found in the "updates" folder of later versions to get to the latest 1.x.x version. I unfortunately don't have time to be maintaining two independent branches with the way extension versions work now in PostgreSQL. Even fixing bugs in the 1.8.x series will be troublesome since I then have to redo a 1.8.x -> 2.x.x upgrade path file every time. At least with only bug fixes, it will be much easier to maintain for the foreseable future. This will unfortunately likely break managing the 1.x.x series via PGXN. I'm sorry but I don't see any other way around this with the way extension versions are managed right now. I highly recommend updating your database to 9.4+ if you wish to easily maintain this extension going forward. There's tons of other great new features and performance improvements, so it's definitely worth the effort!
...
-- A background worker process (BGW) has been added to pg_partman for general partition maintenance.
    ...
    -- New configuration options are listed in the documentation. If you want anything other than the defaults, you must set them in postgresql.conf. Only a reload is required to change all but the maintenance database once the database is started with the BGW running.
    -- The BGW can only be started at cluster start by adding the library to "shared_preload_libraries" in postgresql.conf. This may change later, but it is done this way for initial simplicity and with future design ideas in mind.
...
-- Use new, more extensive GET STACKED DIAGNOSTIC feature added in 9.2 to provide more detailed errors when an exception is encountered. Previously when functions called other functions and a custom exception block was used, only the latest function called would report the error. Now a more full stack trace is available to see the original function that caused the error.

-- TODO to get this uploaded to github, copy all files over to the _v1 folder & remove any old files that shouldn't exist
-- TODO mention in blog post that including this code in the extension update caused all partition trigger functions to be made part of the extension
...
SELECT format('SELECT @extschema@.create_function_id(%L);', parent_table)
FROM @extschema@.part_config
WHERE partition_type = 'id';

INSERT INTO partman_migrate_hybrid_temp 
SELECT format('SELECT @extschema@.create_function_time(%L);', parent_table)
FROM @extschema@.part_config
...
IF v_top_parent IS NOT NULL THEN 
    SELECT partition_interval::bigint INTO v_top_interval FROM @extschema@.part_config WHERE parent_table = v_top_parent;
    v_id_position := (length(p_parent_table) - position('p_' in reverse(p_parent_table))) + 2;
...
IF v_top_parent IS NOT NULL THEN 
    ...
    SELECT partition_interval::interval INTO v_top_interval FROM @extschema@.part_config WHERE parent_table = v_top_parent;
...
LOOP
    ...
    IF v_row.partition_type = 'time' OR v_row.partition_type = 'time-custom' THEN
        ...
        v_premade_count = abs(round(EXTRACT('epoch' FROM age(v_last_partition_timestamp, v_current_partition_timestamp)) / EXTRACT('epoch' FROM v_row.partition_interval::interval)));
        ...
        WHILE v_premade_count < v_row.premake LOOP
            ...
            -- Can be negative when subpartitioning and there are parent partitions in the past compared to current timestamp value.
            -- abs() prevents run_maintenence from running on those old parent tables
            v_premade_count = abs(round(EXTRACT('epoch' FROM age(v_next_partition_timestamp, v_current_partition_timestamp)) / EXTRACT('epoch' FROM v_row.partition_interval::interval)));
        END LOOP;
    ELSIF v_row.partition_type = 'id' THEN
        ...
        -- abs() prevents run_maintenence from running on those old parent tables
        WHILE (abs((v_next_partition_id - v_current_partition_id) / v_row.partition_interval::bigint)) <= v_row.premake 
        LOOP 
